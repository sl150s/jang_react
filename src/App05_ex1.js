...
//여기서 import한 cn은 function으로 생각하고 사용하면된다.
import cn from "classnames";
import binder from "classnames/bind";

//classname binder를 이용해서 모듈화된 css를 바인딩해서 그 참조값을 cx에 대입
    ...
    render() {
        return (
            <div className="container">
                ...
                <br/>
                <button className={
                    cn('btn', this.state.btnColor,
                    {'btn-lg':this.state.isChecked})
                }>색상이 바뀌는 버튼</button>
                ...
                <p className={`${styles["text-red"]} ${styles["text-bold"]} ${styles["text-big"]}`}>p1 입니다.</p>
                <p className={cx('text-red')}>p2 입니다.</p>
                <p className={cx('text-red','text-bold','text-big')}>p3 입니다.</p>
                <p className={cx('text-blue',{'text-bod':true})}>p4 입니다.</p>
            </div>
        );
    }
    onCheckChange=(e)=>{
